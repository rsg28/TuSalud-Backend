...
const { pythonOcrEnabled, extractPdfTextWithPython } = require('./pythonOcrClient');
...
async function extractEmpleadosTextFromPdfBuffer(buffer, opts = {}) {
  ...
  if (pythonOcrEnabled()) {
    try {
      const py = await extractPdfTextWithPython(buffer, {
        maxPages,
        originalname: opts.originalname,
      });
      const pyText = String(py?.text || '').trim();
      if (pyText.replace(/\s/g, '').length >= 40) {
        return {
          text: normalizeExtractedTableText(pyText),
          mode: 'ocr-python',
        };
      }
    } catch (e) {
      // fallback al OCR local actual para no interrumpir el flujo existente
      console.warn('[pdf OCR] Python OCR no disponible, usando OCR local:', e.message);
    }
  }
